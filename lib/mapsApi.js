...
export async function searchPlace(placeName, location) {
  ...
  try {
    // Search for the place name directly, then filter by location if needed
    ...
    // Try to find a result that matches the location area
    for (const result of searchData.results) {
      if (result.formatted_address && result.formatted_address.toLowerCase().includes(location.toLowerCase())) {
        bestMatch = result;
        break;
      }
    }
    
    // Get detailed information including address
    const detailsUrl = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${bestMatch.place_id}&fields=formatted_address,name,rating,user_ratings_total,types&key=${GOOGLE_MAPS_API_KEY}`;
...
export async function validateQuestLocation(quest, location) {
  ...
  if (placeDetails) {
    // Enhance the quest with real location data
    return {
      ...quest,
      location: {
        name: placeDetails.name,

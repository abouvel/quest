...
export class UserService {
  ...
  static async generateUserQuest(userId) {
    try {
      ...
      console.log("Fetching from:", `${process.env.NEXT_PUBLIC_BACKEND_ROUTE}/quests`);

      // Extract coordinates from user data
      let latitude = user.latitude, longitude = user.longitude;
      if (user.latitude && user.longitude) {
        latitude = parseFloat(user.latitude);
        longitude = parseFloat(user.longitude);
        console.log("Using user coordinates:", latitude, longitude);
      }
      console.log("Final coordinates:", latitude, longitude);

      // Call FastAPI /quests endpoint for quest generation
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_ROUTE}/quests`, {
        method: 'POST',
  ...
  static async updateUserCoordinates(userId, latitude, longitude) {
    try {
      const { error } = await supabase
        .from('users')
        .update({
      ...
      console.log('Successfully updated coordinates for user:', userId, 'lat:', latitude, 'lng:', longitude)
      return true
    } catch (error) {
      console.error('Error in updateUserCoordinates:', error)
      return false
    }
  }
} 
...
export class UserService {
  ...
  static async generateUserQuest(userId) {
    try {
      ...
      // Generate quest using AI, passing all titles to exclude
      const quest = await generateQuest(userData, excludedTitles)

      if (quest) {
        // Validate and enhance quest location with real coordinates
        console.log('Validating quest location...')
        const validatedQuest = await validateQuestLocation(quest, location)
        ...
        if (validatedQuest.validated && validatedQuest.location?.coordinates) {
          ...
          quest.lat = validatedQuest.location.coordinates.lat
          quest.lng = validatedQuest.location.coordinates.lng
          quest.locationName = validatedQuest.location.name
          quest.address = validatedQuest.location.address
        } else {
          console.log('Quest location validation failed:', validatedQuest.error)
          // Fallback to city coordinates if location validation fails
          // This would need to be implemented based on your city data
        }

        // Return the quest without saving to database
        console.log('Generated quest (not saved to database yet):', quest)
        return quest
      }

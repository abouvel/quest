...
export const questUtils = {
    ...
    async completeQuest(questId, completionData) {
        try {
            ...
            // Preserve coordinates if they exist and aren't being overridden
            if (quest.lat && quest.lng && !completionData.lat && !completionData.lng) {
    ...
    // Get completed quests with coordinates for map display
    async getCompletedQuestsForMap(userId, filter = 'all') {
        try {
            let query = supabase
                .from('quests')
                .select(`
            ...
            if (filter === 'friends' && userId) {
                const { data: friends } = await userUtils.getFriends(userId);
            ...
            if (error) {
                console.error('Error fetching completed quests for map:', {
            ...
            console.log(`✅ Successfully fetched ${transformedData.length} completed quests for map`);
            return { data: transformedData, error: null };
        } catch (error) {
            console.error('Error in getCompletedQuestsForMap:', error);
            return { data: [], error };
        }
    }
};
